...
data <- read.xlsx("VIAJES_Reducido.xlsx", sheet = 1)
...
get_mann_whitney <- function(data, var) {
  ...
  mw_res <- apply(res, 1, function(row) {
    paste(comparisons[rownames(res)], comparisons[colnames(res)], row, sep = " - ")
  })
...
# Añadir la tabla final proporcionada
tabla_final <- data.frame(
  Variable = c("Promotores (9,10 puntos)", "Neutral (7, 8 puntos)", "Detractores (1 a 6 puntos)"),
  Servicios_aloja_rest = c(47.5, 39.8, 12.7),
  Multiples_atracciones = c(75.7, 19.7, 4.5),
  Turismo_pasivo = c(36.4, 39.4, 24.2),
  Total = c(60.6, 29.6, 9.8),
  Chi2 = 95.938,
  Sig = 0.000
)
...
write.xlsx(list(Resultados = resultados, Tabla_Final = tabla_final), "resultados_segmentacion_turismo_urbano.xlsx")